...
/** Strict, redacted application request for one USER/ADMIN refresh-token grant. */
public record InternalRefreshTokenRequest(
    ...
    public InternalRefreshTokenRequest {
        if (!INTERNAL_CLIENT_IDS.contains(clientId)) {
            throw invalid("clientId must be user or admin");
        }
    ...
    public String toString() {
        return "InternalRefreshTokenRequest[clientId=<redacted>, rawRefreshToken=<redacted>, requestedScopes=<redacted>"
    ...
    private static IllegalArgumentException invalid(String message) {
        return new IllegalArgumentException("Invalid internal refresh-token request: " + message);
    }
}
