...
/** Validated persisted state for one email-code login challenge, without the raw code or email. */
public record EmailLoginCodeState(String clientId, Long userId, String emailFingerprint, String verifierHash,
    ...
    public EmailLoginCodeState {
        if (!"user".equals(clientId) && !"admin".equals(clientId)) throw new IllegalArgumentException("Invalid email-code client");
        if (userId == null || userId <= 0 || emailFingerprint == null || !FINGERPRINT.matcher(emailFingerprint).matches()
                || verifierHash == null || verifierHash.isBlank() || !BCRYPT.matcher(verifierHash).matches()
                || failedAttempts == null || failedAttempts < 0 || failedAttempts > 4 || issuedAt == null || expiresAt == null
                || !expiresAt.isAfter(issuedAt)) throw new IllegalArgumentException("Invalid email-code state");
    }
