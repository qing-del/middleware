...
class EmailLoginCodeStateCodecTest {
    ...
    @Test void roundTripIsStableAndImmutable() {
        Map<String,String> map=codec.encode(state); assertThat(map.keySet()).containsExactlyElementsOf(codec.fieldNames());
    ...
    @Test void invalidSchemaAndFieldsFailClosed() {
        Map<String,String> map=new java.util.LinkedHashMap<>(codec.encode(state)); map.put("schema_version", "2");
