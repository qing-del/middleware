...
public class JwtTokenAdminInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        try {
            ...
            Claims claims = JwtTokenSupport.parseJWT(jwtProperties.getAdminSecretKey(), token);
            Long adminId = Long.valueOf(claims.get(SecurityTokenConstants.ADMIN_ID_CLAIM).toString());
            log.debug("Current admin ID: {}", adminId);

            // 3、通过，接下来校验 redis 中的数据
            String jwt = redis.opsForValue().get(SecurityTokenKeyGenerator.getAdminLoginKey(adminId));
