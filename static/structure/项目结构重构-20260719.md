...
### 阶段 8：Spring Security 兼容迁移计划

阶段 8 是阶段 7 后唯一核心后续阶段；完成条件不是仅增加依赖，而是在不改变既有认证合同的前提下完成请求链与上下文迁移。

- **8.0 身份模型与双写兼容**：沿现有 Spring Boot BOM 引入 `spring-security-core` 的身份/authority 模型。现有三个 MVC JWT 拦截器继续处理请求，同时写入并清理 `SecurityContextHolder`；补充特征测试，暂不启用 `SecurityFilterChain`。
- **8.1 Filter 请求链替换**：引入显式 stateless `SecurityFilterChain` 和 admin/user/activation 三类 Filter，精确保留 admin、user、active 路径与排除项、401 JSON、JWT claim/secret、Redis key/单 Token、激活 URL token、QPS 与清理语义；验证后删除 MVC 拦截器注册。
- **8.2 上下文兼容收口**：`BaseContext`/`PermissionContext` 改为兼容地从 holder 读取；admin/user/activation 分别映射为 `ROLE_ADMIN`、`ROLE_USER`、`ROLE_ACTIVATION`，再逐步停止双 ThreadLocal。
- **8.3 System 登录边界收口**：将 system 登录认证边界收口到安全抽象，但保持登录响应、JWT 格式、TTL、Redis 规则以及退出/激活协议不变。
- **8.4 验证与文档收口**：使用 JDK 21 运行全量验证，并更新迁移记录、兼容基线与剩余风险。

风险控制：Filter 没有 MVC `HandlerMethod` 判定，必须通过请求匹配与 MVC 契约测试证明受保护 Controller 行为；不得默认扩大对静态资源的拦截范围。Spring Security 版本沿用现有 Boot BOM，不升级其他依赖。

### 2026-07-21：阶段 8 Spring Security 兼容迁移完成

- 提交序列：计划 `72ddd79`；8.0 `7a518fd`；8.1 `ee15c3c`；8.2 `7fbc0df`；8.3 `09b9f2d`。
- 显式 stateless `SecurityFilterChain` 与 admin/user/activation 三类 Filter 已替代 MVC interceptors；`HandlerMethod` 判定、路径排除、401 JSON、JWT claim/secret、Redis 单 Token、QPS 与 activation URL 协议均保持兼容。
- 认证成功写入 holder 并映射 `ROLE_ADMIN` authority；`BaseContext`/`PermissionContext` 为 holder-first、legacy fallback。`TokenSessionService` 已封装登录、退出、activation 与 email-change credential，system 三个服务不再依赖 JWT/Redis key 实现。
- JDK 21 `mvn clean verify` 于 2026-07-21 10:40 通过：26/26 Reactor，05:04；security 17、web 2（含真实 MockMvc chain）、system-biz 13（workflow 8）、server 6。
- HTTP、登录响应、JWT/Redis、配置、数据库与前端合同未变。仅保留既有 Mockito dynamic agent 与编译警告。
- 未引入 `UserDetailsService`、`AuthenticationProvider` 或方法授权，未改 BCrypt；`PasswordHashingService`、移除静态 context fallback、Legacy Filter 改名均为可独立推进的后续项，不是当前缺陷。

### 2026-07-21：重构残留目录清理

- 清理依据：对照阶段 6/7/8 的模块基线与当前 Git 跟踪、POM/源码引用；`middleware-pojo`、`middleware-common-legacy` 及四个旧顶层 OSS/Flexmark 目录均不再是 Reactor 模块，未含受跟踪或非 `target` 文件，且没有运行时或构建引用。
- 已移除目录：`middleware-pojo`、`middleware-common-legacy`、`aliyun-oss-spring-boot-autoconfigure`、`aliyun-oss-spring-boot-starter`、`flexmark-jacolp-autoconfigure`、`flexmark-jacolp-starter`；同时移除 common-security 的空 interceptor 包，以及 server 内遗留的空 controller/service/mapper/adapter/task/config 等包目录和空资源目录。
- Docker 构建缓存层已改为根 POM 加当前 25 个子 POM（共 26 个描述符），随后复制当前工程源码并仅构建 `middleware-server`；README 已同步 server 纯装配职责和 Spring Security Filter/`SecurityContext` 鉴权链路。
- 已执行目录存在性、非 `target` 文件、POM/源码旧名称与 `JwtToken*Interceptor` 引用搜索及 `git diff --check` 静态检查。
- JDK 21 `mvn clean verify` 于 2026-07-21 11:16 通过：26/26 Reactor 模块 `BUILD SUCCESS`，总耗时 6 分 31 秒；仅保留既有 Mockito 动态 agent 与编译警告。
