...
<template>
  <div class="flex h-screen w-full overflow-hidden selection:bg-rose-500/30 tracking-tight">

    <!-- 全局环境光晕 -->
    <div class="fixed top-[-10%] right-[-5%] w-[500px] h-[500px] bg-rose-600/10 blur-[150px] rounded-full pointer-events-none z-0"></div>
    <div class="fixed bottom-[-10%] left-[-5%] w-[400px] h-[400px] bg-indigo-600/10 blur-[120px] rounded-full pointer-events-none z-0 animate-blob animation-delay-2000"></div>
    ...
    <aside :class="['h-full glass-panel border-r border-white/5 flex flex-col relative z-20 flex-shrink-0 transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)]', isCollapsed ? 'w-20' : 'w-64']">
      ...
      <!-- 折叠按钮 -->
      <button @click="toggleSidebar" class="absolute -right-3 top-6 w-6 h-6 rounded-full bg-[#020617] border border-white/10 flex items-center justify-center text-slate-400 hover:text-rose-400 hover:border-rose-500/50 hover:shadow-[0_0_10px_rgba(244,63,94,0.3)] transition-all z-30 cursor-pointer">
      ...
      <div class="h-16 flex items-center px-6 border-b border-white/5 overflow-hidden whitespace-nowrap">
        <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-rose-500 to-indigo-600 flex items-center justify-center shadow-[0_0_15px_rgba(244,63,94,0.4)] flex-shrink-0">
          <Shield class="text-white w-4 h-4" />
        </div>
        <h1 :class="['text-xl font-black tracking-tighter text-transparent bg-clip-text bg-[linear-gradient(110deg,#fb7185,45%,#818cf8,55%,#fb7185)] animate-shine ml-2 transition-all duration-300 whitespace-nowrap', isCollapsed ? 'opacity-0 w-0 overflow-hidden' : '']">
          CORE<span class="text-white font-light">NODE</span>
        </h1>
        <span :class="['ml-2 text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 border border-rose-500/50 text-rose-400 bg-rose-500/10 rounded whitespace-nowrap', isCollapsed ? 'opacity-0 w-0 overflow-hidden' : '']">ADMIN</span>
      </div>
      ...
      <div class="p-4 border-t border-white/5 overflow-hidden">
        <button @click="handleLogout" class="glass-logout-btn flex items-center px-4 py-3 rounded-xl text-slate-400 hover:text-rose-400 group whitespace-nowrap w-full">
          <Power class="w-5 h-5 flex-shrink-0 transition-transform group-hover:scale-110 relative z-10" />
          <span :class="['text-sm font-medium tracking-wide relative z-10 ml-3 transition-all duration-300 whitespace-nowrap', isCollapsed ? 'opacity-0 w-0 overflow-hidden' : '']">安全退出 (Logout)</span>
        </button>
      </div>
    </aside>
      ...
      <header class="h-16 glass-panel border-b border-white/5 flex items-center justify-between px-8 z-20 shrink-0">
        <div class="flex items-center space-x-4">
          <div class="flex flex-col">
            <span class="text-sm font-bold text-slate-200">系统管理大盘</span>
            <span class="text-[10px] text-slate-500 uppercase tracking-widest font-semibold mt-0.5">System Dashboard</span>
          </div>
        </div>
        ...
        <div class="flex items-center space-x-6">
          ...
          <div class="flex items-center space-x-3">
            <div class="group relative flex items-center h-8 rounded-full hover:bg-rose-500/10 transition-all duration-300 ease-out overflow-hidden w-8 hover:w-40 cursor-pointer border border-transparent hover:border-rose-500/30">
              <div class="w-8 h-8 flex-shrink-0 flex items-center justify-center text-slate-400 group-hover:text-rose-400 transition-colors z-10 relative">
                <Search class="w-4 h-4" />
              </div>
              <input type="text" placeholder="全局搜索 UID..." class="absolute left-8 w-28 bg-transparent text-[11px] font-bold text-slate-300 outline-none opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-75 placeholder:text-slate-500 uppercase tracking-wider" />
            </div>

            <!-- 通知 -->
            <div class="relative group/bell">
              <button class="w-8 h-8 rounded-full flex items-center justify-center text-slate-400 group-hover/bell:text-rose-400 group-hover/bell:bg-rose-500/10 group-hover/bell:border-rose-500/30 border border-transparent transition-all relative z-10 cursor-pointer">
              ...
              <div class="absolute top-full right-0 pt-3 opacity-0 translate-y-3 pointer-events-none group-hover/bell:opacity-100 group-hover/bell:translate-y-0 group-hover/bell:pointer-events-auto transition-all duration-300 ease-out z-50 origin-top-right w-72">
                <div class="glass-panel rounded-2xl p-2 shadow-2xl border border-white/10 relative overflow-hidden">
                  <div class="absolute -top-10 -right-10 w-24 h-24 bg-rose-500/20 blur-2xl rounded-full pointer-events-none"></div>

                  <div class="flex items-center justify-between px-3 py-2 border-b border-white/5 mb-1 relative z-10">
                    <span class="text-[11px] font-bold text-slate-300 uppercase tracking-wider">System Alerts</span>
                    <span class="text-[9px] text-rose-400 bg-rose-500/10 px-1.5 py-0.5 rounded font-black tracking-widest">3 PENDING</span>
                  </div>
                  ...
                  <div class="max-h-64 overflow-y-auto pr-1 space-y-1 relative z-10">
                    <div class="flex items-start space-x-3 p-2.5 rounded-xl hover:bg-white/[0.06] transition-colors cursor-pointer group/msg border border-transparent hover:border-white/5">
                      <div class="w-6 h-6 rounded-full bg-amber-500/10 flex items-center justify-center flex-shrink-0 mt-0.5 text-amber-400">
                        <ShieldAlertIcon class="w-3.5 h-3.5" />
                      </div>
                      <div class="flex flex-col min-w-0">
                        <span class="text-xs font-semibold text-slate-200 truncate group-hover/msg:text-rose-300 transition-colors">笔记审核请求</span>
                        <span class="text-[10px] text-slate-500 leading-snug mt-0.5 line-clamp-2">用户 (UID: 1024) 提交了新的笔记 "深入理解 React 原理" 审核请求。</span>
                        <span class="text-[9px] text-slate-600 mt-1 font-medium">刚刚</span>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <template v-else-if="authStore.user">
              <div class="flex items-center space-x-3">
                <div class="flex flex-col items-end">
                  ...
                  <span class="text-[10px] font-bold uppercase tracking-widest px-1.5 py-[1px] rounded-full mt-0.5 border border-rose-500/30 text-rose-400 bg-rose-500/10 shadow-[0_0_10px_rgba(244,63,94,0.2)]">Administrator</span>
                </div>
                <div class="relative">
            ...
            <template v-else>
              ...
              <div class="flex items-center space-x-3">
                <div class="flex flex-col items-end">
                  ...
                  <span class="text-[10px] font-bold uppercase tracking-widest px-2 py-[2px] rounded-full mt-0.5 border border-slate-500/30 text-slate-400 bg-slate-500/10">Unknown</span>
                </div>
                <div class="relative">
...
.glass-nav-active {
  ...
  box-shadow: inset 0 1px 1px rgba(255, 255, 255, 0.1), 0 0 15px rgba(244, 63, 94, 0.15);
}
.glass-nav-active i { color: #fb7185; }
